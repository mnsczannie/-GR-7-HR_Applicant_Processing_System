    ...
    public partial class frmInterviewTypes : Form
    {
        ...
        private void LoadData()
        {
            try
            {
                ...
                {
                    string query = "SELECT id, label FROM interview_types";
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = DatabaseHelper.GetConnection())
                {
                    string query = "INSERT INTO interview_types (label) VALUES (@Label)";
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = DatabaseHelper.GetConnection())
                {
                    string query = "UPDATE interview_types SET label = @Label WHERE id = @Id";
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@Label", txtLabel.Text.Trim());
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Remove entry target workflow structure?", "Confirmation", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    using (SqlConnection conn = DatabaseHelper.GetConnection())
                    {
                        string query = "DELETE FROM interview_types WHERE id = @Id";
                        using (SqlCommand cmd = new SqlCommand(query, conn))
                        {
                            cmd.Parameters.AddWithValue("@Id", selectedId);
                            conn.Open();
